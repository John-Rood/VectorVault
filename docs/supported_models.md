...
| xAI | `grok-4.5` (product default) | `grok-4.5` (256k conservative limit), `grok-4.3`, `grok-4.20`, `grok-4.20-0309-reasoning`, `grok-4.20-0309-non-reasoning`, `grok-build-0.1` (256k), `grok-latest` | up to 2,000,000 | model-dependent |
...
xAI's official `grok-latest` alias resolves to `grok-4.3`; Vector Vault deliberately defaults new code/chat work to `grok-4.5` because xAI recommends it for those workloads. xAI's model index currently lists a 500k context for Grok 4.5 while its individual model page lists 256k, so Vector Vault enforces the conservative 256k limit until an authenticated Models API response resolves the discrepancy. Grok 4.20 and both dated `0309` variants use xAI's published 2,000,000-token context limit.
